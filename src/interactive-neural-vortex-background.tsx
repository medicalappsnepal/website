...
const InteractiveNeuralVortex = () => {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      // Limit pixel ratio to 0.75 or 1 to massively improve performance on high-res screens
    ...
    const render = () => {
      ...
      gl.uniform1f(uScrollProgress, scrollY.current / (2 * window.innerHeight));
  ...
  return (
    <canvas 
      ref={canvasRef} 
      id="neuro" 
      className="fixed inset-0 w-full h-full pointer-events-none opacity-30 dark:opacity-60 z-0"
    ></canvas>
  );
};
